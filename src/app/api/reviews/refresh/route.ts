...
async function fetchFromOutscraper(): Promise<Review[]> {
  ...
  try {
    ...
    const response = await fetch(
      `https://api.app.outscraper.com/maps/reviews-v3?query=${encodedQuery}&reviewsLimit=${MAX_REVIEWS}&sort=newest&language=no&async=false`,
      {
        headers: {
          "X-API-KEY": apiKey,
        },
    ...
    if (response.status === 202 && data.results_location) {
      console.log("⏳ Request pending, polling for results...");
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        const pollResponse = await fetch(resultsUrl, {
          headers: {
            "X-API-KEY": apiKey,
          },
        });
        ...
        if (pollData.status === "Success" && pollData.data) {
          ...
          return parseOutscraperData(pollData);
        }
        if (pollData.status === "Error") {
          console.error("❌ Outscraper job failed:", pollData);
          throw new Error("Outscraper job failed");
        }
      }
      throw new Error("Timeout waiting for Outscraper results");
    }
    return parseOutscraperData(data);
  } catch (error) {
    console.error("❌ Error fetching from Outscraper:", error);
    throw error;
  }
}
interface OutscraperData {
  data?: Array<{
    reviews_data?: Array<{
      author_title?: string;
...
function parseOutscraperData(data: OutscraperData): Review[] {
